...
class _ChessBoardScreenState extends State<ChessBoardScreen> {
    ...
    setState(() {
      ...
      if (selectedRow != -1 && selectedCol != -1) {
        ...
        bool moveSuccessful = game.move({
          'from': fromSquare,
          'to': clickedSquare,
          'promotion': 'q' // ඉත්තෙක් අන්තිම කොටුවට ගියොත් ඔටෝමැටිකව රැජිනක් (Queen) බවට පත් කිරීම
        });

        if (!moveSuccessful) {
          // වැරදි මූව් එකක් නම් Notification එකක් පෙන්වීම
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Invalid Move! නිවැරදි චෙස් මූව් එකක් අදින්න.'), duration: Duration(milliseconds: 600)),
          );
        }
        ...
        if (pieceOnClickedSquare != null) {
          ...
          if ((game.turn == chess.Color.WHITE && pieceOnClickedSquare.color == chess.Color.WHITE) ||
              (game.turn == chess.Color.BLACK && pieceOnClickedSquare.color == chess.Color.BLACK)) {
            selectedRow = row;
            selectedCol = col;
          }
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          game.turn == chess.Color.WHITE ? "Japana Chess (White's Turn)" : "Japana Chess (Black's Turn)",
